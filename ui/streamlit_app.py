...
with tab3:
    ...
    if not st.session_state.company:
        ...
        st.success(f"✅ Ready to analyze {len(st.session_state.cvs)} candidates")
        ...
        for idx, cv in enumerate(st.session_state.cvs, 1):
            with st.container():
                ...
                # Mock match score based on experience
                match_score = 75 + (idx % 25) - 10
                
                with col1:
                    if match_score >= 80:
                        st.markdown("<div class='match-high'>🟢 {:.0f}%</div>".format(match_score), unsafe_allow_html=True)
                    elif match_score >= 60:
                        st.markdown("<div class='match-medium'>🟡 {:.0f}%</div>".format(match_score), unsafe_allow_html=True)
                    else:
                        st.markdown("<div class='match-low'>🔴 {:.0f}%</div>".format(match_score), unsafe_allow_html=True)
                
                with col2:
                    st.markdown(f"### {cv.get('name', 'N/A')}")
                    st.write(f"📚 {cv.get('education', 'N/A')} | {cv.get('degree', 'N/A')}")
                    st.write(f"⏱️ {cv.get('years_experience', 0)} years experience")
                
                with col3:
                    if st.button("👁️ View Details", key=f"details_{idx}"):
                ...
                if st.session_state.get(f"show_details_{idx}", False):
                    with st.expander("📋 Detailed Feedback", expanded=True):
                        ...
                        col_a.metric("Technical", "85%")
                        col_b.metric("Soft Skills", "72%")
                        col_c.metric("Culture Fit", "68%")
                        col_d.metric("CV Quality", "90%")
                        
                        st.markdown("**Why This Match:**")
                        st.write(f"- Strong technical background with {cv.get('years_experience', 0)} years experience")
                        st.write("- Demonstrated leadership in previous roles")
                        st.write("- Communication skills evident from diverse experience")
                        
                        st.markdown("**Growth Areas:**")
                        st.write("- Limited ownership in decision-making roles")
                        st.write("- More examples of adaptability needed")
                        st.write("- CV could use more quantified metrics")
                        
                        st.markdown("**Top 3 Improvements:**")
                        st.write("1. **Quantify impact**: Add metrics to all achievements")
                        st.write("2. **Show leadership**: Highlight team leadership examples")
                        st.write("3. **Culture alignment**: Emphasize collaborative approach")
